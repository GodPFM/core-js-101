 ...
 * @param obj
 */
function returnCoordinates(obj) {
  const array = [];
...
function doRectanglesOverlap(rect1, rect2) {
  const firstRectangle = returnCoordinates(rect1);
  const secondRectangle = returnCoordinates(rect2);
  let result;
  firstRectangle.map((item) => {
    if (secondRectangle.includes(item)) {
      result = true;
    }
    return item;
  });
...
function getCommonDirectoryPath(pathes) {
  let res = '';
  const arrayOfFirst = pathes[0].split('/');
  for (let i = 1; i < arrayOfFirst.length; i += 1) {
    let flag = false;
    let tepm = `${arrayOfFirst.slice(0, i).join('/')}`;
    if (tepm === '') {
      tepm = '/';
    }
    for (let j = 1; j < pathes.length; j += 1) {
      let template = pathes[j].split('/').slice(0, i).join('/');
      if (template === '') {
        template = '/';
      }
      flag = template === tepm;
  ...
  if (pathes[0].at(res.length) === '/' && res.length !== 0) {
    res += '/';
  }
  return `${res}`;
}
